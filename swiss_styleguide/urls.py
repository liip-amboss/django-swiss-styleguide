...
urlpatterns = [
    ...
    url(r'^auth/login/$', auth_views.LoginView.as_view(
            template_name='swiss_auth/login.html'
        ), name='login'),
    ...
    url(r'^auth/password_change/done/$',
    ...
    url(r'^auth/password_reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',  # noqa
